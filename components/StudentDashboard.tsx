...
export const StudentDashboard: React.FC<Props> = ({ user, dailyStudySeconds, onSubjectSelect, onRedeemSuccess, settings, onStartWeeklyTest, activeTab, onTabChange, setFullScreen, onNavigate, isImpersonating, onNavigateToChapter }) => {
  ...
  const [mainBannerIndex, setMainBannerIndex] = useState(0); // NEW: Consolidated Banner Index
  ...
  useEffect(() => {
      ...
      }, 5000); // 5s Interval for Hero internal slides
      ...
      if (activeTab === 'HOME') { 
          ...
          return (
              <div className="space-y-6 pb-24">
                ...
                {/* HERO SECTION - CONSOLIDATED BANNER CAROUSEL */}
                  <div className="space-y-4 mb-6">
                      ...
                      {(() => {
                          const bannerComponents = [
                              // 1. HERO SLIDER (App Features)
                              <div key="hero" className="relative h-48 rounded-2xl overflow-hidden shadow-xl mx-1 border border-white/20" style={{ backgroundColor: 'var(--primary)' }}>
                              ...
                              <div 
                                  ...
                                  className="mx-1 h-48 bg-gradient-to-r from-orange-500 to-red-500 p-4 rounded-2xl shadow-lg text-white flex items-center justify-between cursor-pointer border border-white/20 relative overflow-hidden group"
                              >
                                  <div className="relative z-10">
                              ...
                              // 4. MORNING INSIGHT
                              morningBanner ? (
                                  <div key="morning" className="mx-1 h-48 bg-gradient-to-r from-orange-100 to-amber-100 p-4 rounded-2xl shadow-sm border border-orange-200 overflow-y-auto">
                              ...
                              // 5. PROMO BANNER
                              <div key="promo" onClick={() => onTabChange('STORE')} className="mx-1 h-48 bg-gradient-to-r from-slate-900 to-slate-800 p-4 rounded-2xl shadow-lg flex items-center justify-between cursor-pointer border border-slate-700 relative overflow-hidden group">
                                  <div className="relative z-10">
                                      <div className="flex items-center gap-2 mb-1">
                          ...
                          if (bannerComponents.length === 0) return null;

                          return (
                              <div className="relative mx-1 mb-6 group">
                                  {/* CURRENT BANNER */}
                                  <div className="transition-all duration-500 ease-in-out">
                                      {bannerComponents[mainBannerIndex % bannerComponents.length]}
                                  </div>

                                  {/* NAVIGATION CONTROLS */}
                                  <div className="flex justify-center items-center gap-4 mt-2">
                                      <button 
                                          onClick={(e) => { e.stopPropagation(); setMainBannerIndex(prev => (prev - 1 + bannerComponents.length) % bannerComponents.length); }}
                                      ...
                                      <div className="flex gap-1.5">
                                          {bannerComponents.map((_, idx) => (
                                              <div 
                                                  key={idx} 
                                                  className={`h-1.5 rounded-full transition-all duration-300 ${idx === (mainBannerIndex % bannerComponents.length) ? 'w-6 bg-slate-800' : 'w-1.5 bg-slate-300'}`}
                                              />
                                          ))}
                                      </div>

                                      <button 
                                          onClick={(e) => { e.stopPropagation(); setMainBannerIndex(prev => (prev + 1) % bannerComponents.length); }}
                                          className="p-1.5 rounded-full bg-slate-200 hover:bg-slate-300 text-slate-600 transition-colors"
                                      >
                                          <ArrowRight size={16} />
                                      </button>
    ...
    <div className="min-h-screen bg-slate-50 pb-20">
        ...
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-200 shadow-lg z-50 pb-safe">
            <div className="flex justify-around items-center h-16">
                ...
                <button onClick={() => { onTabChange('COURSES'); setContentViewStep('SUBJECTS'); }} className={`flex flex-col items-center justify-center w-full h-full ${activeTab === 'COURSES' || (activeTab === 'VIDEO') || activeTab === 'PDF' || activeTab === 'MCQ' || activeTab === 'AUDIO' ? 'text-blue-600' : 'text-slate-400'}`}>
                    <Book size={24} fill={activeTab === 'COURSES' || (activeTab === 'VIDEO') || activeTab === 'PDF' || activeTab === 'MCQ' || activeTab === 'AUDIO' ? "currentColor" : "none"} />
                    <span className="text-[10px] font-bold mt-1">Courses</span>
                </button>

                {/* AI TUTOR BUTTON (Relocated) */}
                <button 
                    onClick={() => setAiAssistantOpen(true)}
                    className={`flex flex-col items-center justify-center w-full h-full ${aiAssistantOpen ? 'text-indigo-600' : 'text-slate-400'}`}
                >
                    <div className="relative">
                         <Bot size={28} className={aiAssistantOpen ? "text-indigo-600" : "text-slate-400"} />
